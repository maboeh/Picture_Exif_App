...
class Gui:
    ...
    def create_widgets(self):
        ...
        self.sourceBrowse = ttk.Button(
            self.root, text="Browse", command=self.browseSource)
        self.sourceBrowse.grid(
            row=1, column=3, sticky="nsew", padx=10, pady=10)

        self.sourceCheck = ttk.Checkbutton(
            self.root, text="include Subfolders", variable=self.sourceCheck_var)
        self.sourceCheck.grid(row=2, column=2, sticky="w", pady=10)

        # Dropdown
        self.optionDrop = ttk.Combobox(
            self.root, values=["Date and Time", "Author", "Place"])
        ...
        self.targetBrowse = ttk.Button(
            self.root, text="Browse", command=self.browseTarget)
        self.targetBrowse.grid(
            row=6, column=3, sticky="nsew", padx=10, pady=10)

        self.targetCheck = ttk.Checkbutton(
            self.root, text="generate CSV-file", variable=self.targetCheck_var)
        self.targetCheck.grid(row=7, column=2, sticky="w", pady=10)

        # Textausgabe
        self.textWidget = ScrolledText(self.root, height=5, width=50)
        self.textWidget.grid(row=8, column=1, columnspan=5,
                             sticky="nsew", padx=10, pady=10)

        # Submit
        self.submitButton = ttk.Button(
            self.root, text="Submit", command=self.submit)
        self.submitButton.grid(row=9, column=1, columnspan=3,
                               sticky="nsew", padx=10, pady=10)
    ...
    def submit(self):
        ...
        logging.info(
            f'Source folder: {source_folder}, Target folder: {target_folder}')
        ...
        if self.sourceCheck_var.get():
            logging.info(
                'Die if-Schleife für sourceCheck_var.get() wurde ausgeführt')
